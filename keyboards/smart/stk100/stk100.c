...
extern int st_wireless_init(void);
extern int st_wireless_start(uint8_t ble_channel);
extern int st_wireless_stop(void);
extern int st_wireless_delete(uint8_t ble_channel);
...
#define MODE_BLE 1
#define MODE_24G 2
uint8_t mode_now = MODE_USB;

bool dip_switch_update_mask_kb(uint32_t state) { 
    switch (state&0x3)
    {
        case MODE_USB:
            if(mode_now!=MODE_USB)
                st_wireless_stop();
            mode_now=MODE_USB;
            break;
        case MODE_BLE:
            if(mode_now!=MODE_BLE)
                st_wireless_start(0);
            mode_now=MODE_BLE;
            break;
        case MODE_24G:
            if(mode_now!=MODE_24G)
                st_wireless_start(1);
            mode_now=MODE_24G;
            break;
        default:
